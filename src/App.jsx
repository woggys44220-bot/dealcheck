...
function FlipMode({ onBack, onSaveHistory }) {
  ...
  const useSuggestions = () => {
    if (!aiSuggestion) return;
    setForm((prev) => ({ ...prev, name: safeText(aiSuggestion.objectName || aiSuggestion.visibleListingTitle, prev.name), category: categories.includes(aiSuggestion.category) ? aiSuggestion.category : prev.category, condition: conditions.includes(aiSuggestion.condition) ? aiSuggestion.condition : prev.condition, ask: aiSuggestion.detectedPrice != null && Number.isFinite(Number(aiSuggestion.detectedPrice)) ? String(aiSuggestion.detectedPrice) : prev.ask, city: safeText(aiSuggestion.detectedCity, prev.city) }));
