...
function computeSell(form) { const base = Number(form.value)||0; const coef = conditionCoef[form.condition]; const advised = base*coef; const quick = advised*0.8; const high = advised*1.25; const ease = easeByCategory[form.category]; let score = 50 + (coef*35) + easeScore[ease]; if (form.objective==='vendre vite') score += 5; if (form.objective==='vendre au meilleur prix') score -= 2; score = clamp(score);
  let decision = score >= 75 ? 'PRIX CORRECT' : score >= 55 ? (lotCategories.has(form.category) ? 'VENDS EN LOT' : 'VENDS VITE') : 'BAISSE LE PRIX';
  if (form.objective==='vendre au meilleur prix' && score >= 75) decision = 'ATTENDS LE BON ACHETEUR';
  const strategy = lotCategories.has(form.category) ? 'Vends en lot de 3 pour augmenter la valeur perçue.' : form.objective === 'vendre au meilleur prix' ? 'Commence au prix haut pendant 5 jours puis baisse si personne ne répond.' : 'Prix trop ambitieux, vise la vente rapide.';
  ...
  return { quick, advised, high, score, ease: level, strategy, decision, title, description };
}
...
const Score = ({ score }) => <div className="score"><div className="bar" style={{width:`${score}%`}} /><span>Score {Math.round(score)}/100</span></div>;
